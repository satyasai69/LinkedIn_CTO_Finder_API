...
class LinkedInProfileSearchService {
  ...
  async searchCtoProfiles(
    ...
    const executiveTitles = [
      ...
      'Head of Technology', 'Technology Director', 'VP Engineering',
      'Chief Technical Officer', 'Head of Engineering', 'Tech Lead',
      'Engineering Director', 'Technology VP', 'Chief Technology',
      'VP of Technology', 'VP of Engineering', 'Head of Tech'
    ];
    ...
    const titleQuery = executiveTitles.map(title => `"${title}"`).join(' OR ');
    ...
    if (companySector) {
      ...
      const sectorTerms = sectorKeywords[companySector.toLowerCase()] || [companySector];
    ...
    if (companyType) {
      ...
      const typeTerms = typeKeywords[companyType.toLowerCase()] || [companyType];
      const typeQuery = typeTerms.map(term => `"${term}"`).join(' OR ');
      queryParts.push(`(${typeQuery})`);
    }

    // Exclude common non-executive terms
    const excludes = ['Intern', 'Student', 'Former', 'Ex-', 'Previous', 'Consultant'];
  ...
  private async executeSearch(query: string, numResults: number): Promise<GoogleSearchResponse> {
  ...
  private async executeSearchAllPages(query: string): Promise<GoogleSearchResponse> {
    ...
    while (hasMoreResults) {
      ...
      try {
        ...
        if (data.items && data.items.length > 0) {
          ...
          console.log(`Found ${data.items.length} items on page starting at ${startIndex}. Total so far: ${allItems.length}`);
...
class SerpApiSearchService {
  ...
  private async executeSearch(query: string, numResults: number): Promise<SerpApiResponse> {
    try {
      const params = new URLSearchParams({
        api_key: this.apiKey,
        engine: 'google',
  ...
  private async executeSearchAllPages(query: string): Promise<SerpApiResponse> {
    ...
    for (let page = 0; page < 10; page++) {
      try {
        ...
        const response = await fetch(`${this.baseUrl}?${params}`);
        if (!response.ok) {
          console.error(`SerpAPI request failed for page ${page + 1}: ${response.status}`);
          break;
        }
        ...
        console.log(`SerpAPI: Fetched page ${page + 1}, got ${data.organic_results.length} results`);
...
class ProfileExtractor {
  ...
  constructor() {
    ...
    this.executiveTitles = [
      ...
      'head of technology', 'technology director', 'vp engineering',
      'chief technical officer', 'head of engineering', 'tech lead',
      'engineering director', 'technology vp'
    ];
  }

  extractProfiles(searchResponse: GoogleSearchResponse, searchContext: string = ''): LinkedInProfile[] {
  ...
  extractProfilesFromSerpApi(searchResponse: SerpApiResponse, searchContext: string = ''): LinkedInProfile[] {
  ...
  private parseSearchItem(item: GoogleSearchItem, searchContext: string): LinkedInProfile | null {
    const url = item.link || '';
    ...
    const confidence = this.calculateConfidence(jobTitle, company, searchContext, snippet);
  ...
  private extractTitleAndCompany(title: string, snippet: string): [string, string] {
  ...
  private calculateConfidence(jobTitle: string, company: string, searchContext: string, snippet: string): number {
    let score = 0.0;

    // Title relevance (0-40 points)
    if (this.executiveTitles.some(execTitle => jobTitle.toLowerCase().includes(execTitle))) {
      score += 40.0;
    }

    // Context match (0-30 points)
    if (searchContext && company.toLowerCase().includes(searchContext.toLowerCase())) {
      score += 30.0;
    } else if (searchContext && snippet.toLowerCase().includes(searchContext.toLowerCase())) {
      score += 20.0;
    }
...
// Service instances
const searchService = new LinkedInProfileSearchService(GOOGLE_API_KEY, GOOGLE_CSE_ID);
...
app.post('/api/search/profiles', async (c) => {
  try {
    ...
    const getAllPages = request.get_all_pages !== undefined ? request.get_all_pages : !request.num_results;
    ...
    const searchContext = [request.region, request.company_sector, request.company_type]
      .filter(Boolean)
      .join(' ');
    const profiles = profileExtractor.extractProfiles(searchResults, searchContext);
...
app.post('/api/search/profiles-serpapi', async (c) => {
  try {
    ...
    const searchContext = [request.region, request.company_sector, request.company_type]
      .filter(Boolean)
      .join(' ');
    const profiles = profileExtractor.extractProfilesFromSerpApi(searchResults, searchContext);
